...
from database import read_note_from_db,read_note,write_note_into_db, delete_note_from_db, match_user_id_with_note_id
from database import image_upload_record,list_images,list_images_for_user, match_user_id_with_image_uid, delete_image_from_db
...
def FUN_public():
    ...
    #print(images_table["image_id"])
    return render_template("public_page.html",images = images_table,notes=notes_table)
@app.route("/private/")
def FUN_private():
...
def FUN_upload_image():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            image_upload_record(image_uid, session['current_user'], filename, upload_time,description)
            return(redirect(url_for("FUN_private")))
...
def FUN_add_user():
    if session.get("current_user", None) == "ADMIN": # only Admin should be able to add user.
        # before we add the user, we need to ensure this is doesn't exist in database. We also need to ensure the id is valid.
